(ns agora.kotoba-paginate-qualification-test
  (:require [agora.main :as oracle]
...
(def source-path "src/agora/paginate.kotoba")
...
(deftest q9-paginate-kernel-oracle-and-backends-agree
  (let [source (slurp source-path)
    ...
    (is (true? (boolean (oracle/has-more-kernel? 101 (oracle/clamp-limit (oracle/as-int-kernel 100))))))
    (is (= 1 (if (true? reference-result) 1 reference-result)
             (if (true? compiler-result) 1 compiler-result)))
