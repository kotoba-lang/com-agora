(ns agora.main
  "Kotodama WASM entrypoint for the Agora clean-room actor (L4) — Clojure port.
  ...
  facts (`agora.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(defn as-int-kernel
  ...
  of agora.main/as-int stays here as the retained oracle."
  [v]
  (if (pos? v) v 0))
...
(defn emit-facts
  "EAVT facts for one record: {\"agora.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "agora-compat" :tier tier :entities entities} 200])
